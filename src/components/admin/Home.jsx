...
export default function Home() {
  ...
  const getAllDocuments = async (
    ...
    try {
      ...
      const totalCount =
        data?.totalCount ?? data?.totalItems ?? data?.count ?? null;
  ...
  const handleUpload = async () => {
    ...
    if (
      ...
      notify(
        "A document with this filename or content already exists.",
        "error",
      );
      return;
    }
        ...
        >
          ...
          <div className="flex items-center justify-between px-5 py-4 border-t border-white/10">
            ...
            <button
              className="rounded-lg border border-white/15 bg-white/10 px-3 py-1.5 text-sm text-white/90 disabled:opacity-40"
              disabled={
                totalPages > 1 ? pageNumber >= totalPages : !hasNextPage
              }
            ...
            <div className="w-full p-8 px-10 pb-16 bg-[#00092d] backdrop-blur-lg rounded-2xl border border-white/10 shadow-2xl">
              <div className="flex justify-between pb-3 mb-4">
                <h1 className="text-2xl font-bold text-white">
                  ...
                  {mode === "producstandservices" &&
                    "Upload Products and Services Files"}
                  {mode === "report"}
                </h1>
              </div>
                ...
                <div className="flex-row justify-between gap-6">
                  ...
                  <div className="flex w-full">
                    {error && (
                      <p
                        className={`text-red-600 text-sm mt-2 font-medium
									bg-white p-1.5 px-2 rounded-b-lg`}
                      >
                        {error}
                      </p>
                    )}
                    ...
                    <button
                      ...
                      disabled={uploading}
                      className="w-full mt-6 bg-white/90 hover:bg-white/25 text-[#183398] hover:text-white text-md font-bold py-2 px-4 rounded-full transition"
                    >
                      <span>
                        {uploading ? (
              ...
              {mode === "models" && (
                <ManageAiModels
                  ...
                  clearEditingRow={() => setEditingRow(null)}
                />
              )}
              ...
              {mode === "producstandservices" && (
                <ManageProductsandServices
                  ...
                  notify={notify}
                  jobs={jobs}
                  onUploaded={getAllProductDocuments}
                  error={error}
                />
              )}
              ...
              {mode === "report" && (
                <div className="w-full space-y-6">
                  ...
                  {reportMode === "dashboard" && <Dashboard />}

                  {reportMode === "querylogs" && <QueryLogs />}

                  {reportMode === "documentlogs" && <DocumentLogs />}

                  {reportMode === "graphs" && <Graphs />}

                  {reportMode === "calculator" && <Calculator />}
                </div>
              )}
            </div>
