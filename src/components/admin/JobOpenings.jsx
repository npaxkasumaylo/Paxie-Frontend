    ...
    <>
      ...
      >
        <div className="flex items-center justify-between pb-3 mb-4 border-b-2 border-b-white/50">
          <h1 className="text-2xl font-bold text-white pb-3 ">Job List</h1>
        </div>
        {jobs && jobs.length > 0 ? (
          jobs.map((item) => (
            <div key={item.id} className=" mt-2.5 relative group">
              <div className="flex p-2 bg-white/10 rounded-lg border border-white/10 hover:bg-white/20 cursor-pointer">
                ...
                >
                  {deleting && currentId === item.id ? (
                    ...
                    <CircleX
                      size={20}
                      className=" text-white/90 ext-white/90 transition-transform duration-200 hover:scale-125"
                    />
                  )}
                </button>
              </div>
              ...
              >
                <p className="text-white/90 text-sm">
                  <b>Description:</b> {item.jobDescription}
                </p>
                <p className="text-white/90 text-sm">
                  <b>Requirements:</b> {item.jobRequirements}
                </p>
              </div>
            </div>
          ))
